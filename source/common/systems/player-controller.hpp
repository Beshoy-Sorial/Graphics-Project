    ...
    for (auto torso : world->getEntities()) {
      ...
      if (fighter->state == FighterState::KNOCKED_DOWN) {
        ...
        if (fighter->isPlayer) {
          ...
          if (rand() % 100 < 5) { // ~3 clicks per second on average at 60fps
            fighter->recoveryClicks++;
          }
          int required = 25 + (fighter->knockdownCount - 1) * 15;
          if (fighter->recoveryClicks >= required) {
        ...
        if (fighter->stunnedTimer > 0.0f) {
          ...
          if (fighter->isPlayer) {
            ...
            if (dist > 1.2f) {
              ...
              // AI is in range, make a combat decision based on timer
              ...
              if (fighter->aiDecisionTimer <= 0.0f) {
                ...
                if (choice == 0) {
                  ...
                  if (audioEngine)
                    ma_engine_play_sound(audioEngine, "assets/audio/bunch.mp3",
                                         NULL);

                  // Hit/Block detection for AI punch
                  float hitDist = glm::distance(fighter->basePosition,
                                                playerFighter->basePosition);
                  if (hitDist <= 1.5f) {
                    // Directional blocking logic:
                    // If AI punches with LEFT, player must block with RIGHT
                    // (LMB mirror) If AI punches with RIGHT, player must block
                    // with LEFT (RMB mirror)
                    bool aiPunchedLeft =
                        !fighter->nextPunchLeft; // We flipped it after
                                                 // triggering punch

                    bool blockedCorrectSide = false;
                    if (playerFighter->isDefending) {
                      if (aiPunchedLeft) {
                        // AI Left -> Player Right (RMB)
                        if (mouse.isPressed(GLFW_MOUSE_BUTTON_RIGHT))
                          blockedCorrectSide = true;
                      } else {
                        // AI Right -> Player Left (LMB)
                    ...
                    if (blockedCorrectSide) {
                      // Blocked! AI gets stunned
                      fighter->stunnedTimer = FighterComponent::STUN_DURATION;
                      if (audioEngine)
                        ma_engine_play_sound(audioEngine,
                                             "assets/audio/stun.mp3", NULL);
                    } else if (playerFighter->state !=
                               FighterState::KNOCKED_DOWN) {
                      // Hit lands! (Wrong block side or no block)
                  ...
                  fighter->aiDecisionTimer = (0.5f + (rand() % 10) * 0.05f) / fighter->speedMultiplier;
                } else if (choice == 1) {
                  // DEFEND
                  fighter->isDefending = true;
                  fighter->aiDecisionTimer = 1.0f + (rand() % 10) * 0.1f;
                } else {
                  // IDLE
                  fighter->isDefending = false;
                  fighter->aiDecisionTimer = 0.5f + (rand() % 10) * 0.1f;
                }
              }
            }
          }
      ...
      if (isMoving) {
        ...
        // If AI, rotate to face player even when standing still
        if (!fighter->isPlayer && !isReferee &&
            fighter->state != FighterState::KNOCKED_DOWN) {
          float dx = playerFighter->basePosition.x - fighter->basePosition.x;
          float dz = playerFighter->basePosition.z - fighter->basePosition.z;
      ...
      for (auto child : world->getEntities()) {
        ...
        if (child->name.find("Head") != std::string::npos) {
          ...
          if (fighter->isPlayer && isFirstPerson) {
            ...
            child->localTransform.scale =
                glm::vec3(0.191f); // Original head scale
          }

          auto *mr = child->getComponent<MeshRendererComponent>();
          if (mr) {
            Material *skinMat = AssetLoader<Material>::get(fighter->skinMaterialName);
            if (!skinMat) skinMat = AssetLoader<Material>::get("skin"); // Fallback
